...
#
# Unless required by applicable law or agreed to in writing, softwareeImageProcessor, BatchFeature, get_size_dict
...
from utils.doc import add_start_docstrings,  add_start_docstrings_to_model_forward

# from transformers import ACT2FN

# from activations import ACT2FN
from packets.activations import ACT2FN
from packets.modeling_outputs import BaseModelOutput
...
allows indexing by integer 
or slice (like a tuple) or strings (like a dictionary) that will 
...
class SamPatchEmbeddings(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.projection = nn.Conv2d(num_channels, hidden_size, kernel_size=patch_size, stride=patch_size) # (3, 768, 16, 16) ->([1, 64, 64, 768])
        

    def forward(self, pixel_values):
        batch_size, num_channels, height, width = pixel_values.shape

        # print(self.num_channels, self.config.hidden_size,self.patch_size )   3 768 (16, 16)
        ...
        embeddings = self.projection(pixel_values).permute(0, 2, 3, 1) #torch.Size([1, 64, 64, 768])
        return embeddings
...
class SamVisionEncoder(nn.Module):
    def __init__(self, config: SamVisionConfig):
        ...
        self.layers = nn.ModuleList()
        for i in range(config.num_hidden_layers): # num_hidden_layers = 12  configuration_sam.num_hidden_layers = 12
            # -----------------------------------------------------------
            # print('asdbsafdsa -test',config.num_hidden_layers)
            # -----------------------------------------------------------
            layer = SamVisionLayer(
                config,
                window_size=config.window_size if i not in config.global_attn_indexes else 0, # window_size=14,global_attn_indexes=[2, 5, 8, 11],
            )
    ...
    def forward(
        ...
        output_attentions = output_attentions if output_attentions is not None else self.config.output_attentions
        # -----------------------------------------------------------
        # print('asdbsafdsa -test self.config.output_attentions' ,self.config.output_attentions,output_attentions) #FALSE FALSE
        # -----------------------------------------------------------
        output_hidden_states = (
            output_hidden_states if output_hidden_states is not None else self.config.output_hidden_states
        )
        ...
        Transformer.
        """
        hidden_states = self.patch_embed(pixel_values) # hidden_states torch.Size([1, 64, 64, 768])
        
        
        # -----------------------------------------------------------
        # print('asdbsafdsa -test hidden_states' ,hidden_states.shape) #  torch.Size([1, 64, 64, 768]
        # -----------------------------------------------------------
        

        if self.pos_embed is not None:
            # print('self.pos_embed',self.pos_embed.shape,self.pos_embed) #torch.Size([1, 64, 64, 768]), self.pos_embed全为0 
        ...
        '''
        self.layers 有 num_hidden_layers = 12 个 SamVisionLayer nn.ModuleList()
        '''
...
class SamModel(SamPreTrainedModel):
    ...
    def __init__(self, config):
        ...
        https://huggingface.co/facebook/sam-vit-huge/blob/main/config.json
        '''
    ...
    def get_image_embeddings(
        ...
        # print('type vision_encoder',type(vision_output))
